...
async function getOnlineList() {
  try {
    ...
    if (!list) return console.error("user list not found");
    ...
    const text = await res.text();
    alert("Online list failed: " + text);
  } catch (e) {
    console.error(`List failed`, e);
    alert("List error, check console");
  }
}
function displayMessage(message) {
  const chatContent = document.getElementById(`chat-content`),
    timeText = new Date(message.time).toLocaleTimeString(`en-US`, {
      hour12: false,
    });
